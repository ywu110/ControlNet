...
def infer(args):
    ...
    mnist_canny = MnistDataset('test', im_path=dataset_config['im_test_path'], use_condition=True)
    ...
    model.load_state_dict(torch.load(os.path.join(train_config['task_name'], train_config['controlnet_ckpt_name']), map_location=device))
    model.eval()
